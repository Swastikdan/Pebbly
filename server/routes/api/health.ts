...
let cachedDbCheck: { result: CheckResult; ok: boolean; timestamp: number } | null = null;
...
export default defineEventHandler(async (event) => {
	...
	} else if (cachedDbCheck && now - cachedDbCheck.timestamp < DB_CHECK_CACHE_TTL_MS) {
	...
	if (!ok) {
		...
		setResponseHeader(event, "Cache-Control", "public, max-age=10, s-maxage=10");
	}
