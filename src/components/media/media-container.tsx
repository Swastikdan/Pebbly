        ...
        {hasBackdrops && (
          <TabsPanel value="backdrops">
            <MediaThumbRail
              ...
              lightboxContentClassName="aspect-video w-full max-w-[90vw] rounded-lg border-0 bg-neutral-950 p-0 ring-0 gap-0 overflow-hidden"
              getLightboxTitle={() => `${title} Backdrop Image`}
              renderLightboxBody={(image) => (
                <div className="relative isolate z-1 size-full h-full overflow-hidden rounded-lg bg-neutral-950 p-0">
                  <Image
                    alt={title}
                    className="aspect-video size-full rounded-lg object-cover"
                    height={300}
                    src={image.backdrop_image_raw ?? ""}
                    blurSrc={image.backdrop_image ?? undefined}
                    width={450}
                  />
                </div>
              )}
        ...
        {hasPosters && (
          <TabsPanel value="posters">
            <MediaThumbRail
              ...
              lightboxContentClassName="aspect-[2/3] w-[85vw] max-h-[90vh] rounded-2xl border-0 bg-neutral-950 p-0 ring-0 gap-0 overflow-hidden sm:w-auto sm:h-[90vh] sm:max-h-none"
              lightboxOverlayClassName="bg-black/80 backdrop-blur-sm dark:bg-black/85"
              getLightboxTitle={() => `${title} Poster Image`}
              renderLightboxBody={(image) => (
                <div className="relative isolate z-1 size-full h-full overflow-hidden rounded-2xl bg-neutral-950 p-0">
                  <Image
                    alt={title}
                    className="aspect-2/3 size-full rounded-2xl object-cover"
                    height={300}
                    src={image.poster_image_raw ?? ""}
                    blurSrc={image.poster_image ?? undefined}
                    width={450}
                  />
                </div>
              )}
