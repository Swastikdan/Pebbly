      ...
      {isLoading ? (
        ...
        <div className="bg-secondary relative h-48 w-full overflow-hidden rounded-lg border border-black/8 md:h-52 lg:h-60 dark:border-white/8">
          ...
          <div className="relative flex h-full flex-col items-start justify-center p-5">
            <span className="font-heading text-lg font-semibold text-white md:text-xl lg:text-2xl xl:text-3xl">
              Part of the {data?.name}
            </span>
            <span className="mt-2 flex flex-wrap text-xs text-white/90 md:text-sm lg:text-base">
              Includes{" "}
              {data?.parts
                ?.slice(0, 5)
                .map((part) => part.title)
            ...
            >
              <Button
                variant={null}
                size="lg"
                className="pressable mt-3 border border-black/10 bg-white font-medium text-black shadow-none hover:bg-white/90 hover:text-black active:bg-white/80 dark:border-white/15 dark:bg-white dark:text-black dark:hover:bg-white/90"
              >
                View Collection
              </Button>
            </Link>
