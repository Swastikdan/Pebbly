...
/**
 * The Worker `env` binding. On Cloudflare, Nitro's cloudflare-module handler
 * sets `globalThis.__env__` on every request; Node-based local dev falls back
 * to `process.env`.
 */
...
/** True for production and local dev (anything not preview). */
