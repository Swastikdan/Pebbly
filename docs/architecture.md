...
| Styling | Tailwind CSS v4, Radix UI, shadcn-style components | `src/components/ui/`, `src/styles.css` |
...
### 3.2 Server functions (`src/server/fns/`) — the "API"
...
| `lists.ts` | Custom-list CRUD + list-item toggling, enriched list reads |
...
- `schema.ts` — the Drizzle SQLite schema (all 10 tables, indexes, checks; see
...
- `helpers/` — shared DB helpers: `watch-item.ts` (upsert/normalize) and
  `snapshots.ts` (watchlist snapshot creation).
...
- `src/components/ui/` — shadcn-style primitives (button, dialog, dropdown,
  tabs, select, sheet, toaster, image, etc.).
- `src/components/media/` — movie/TV detail-page blocks (cast, seasons,
  episode browser, lightbox, watchlist status menu, player).
- `src/components/watchlist/`, `src/components/recommendations/`,
...
- `src/components/media-card.tsx`, `homepage-media.tsx`,
  ...
  cross-cutting discovery widgets.
...
4. The watchlist button queries `getMediaState` (a server fn) per media id —
   this returns the user's `watch_items` row for that title, or `null`.
...
- **GitHub Actions** (`.github/workflows/deploy.yml`): on push to
  `cloudflare` / `feat/convex-to-traditional-backend` (or manual dispatch) —
  checkout → pnpm setup → Node 22 → install → typecheck → apply D1 migrations
  (prod) → build with `VITE_*` secrets → `wrangler deploy`. Actions are pinned
  to node24-compatible versions (checkout v7, setup-node v7, pnpm/action-setup
  v6, wrangler-action v4) so the workflow is not affected by the Node 20
  deprecation on GitHub runners.
- **Cron**: Cloudflare Cron Trigger `0 3 * * *` → Nitro task `snapshots`
  (`server/tasks/snapshots.ts`) → `createDailySnapshots`, bounded per run and
  resumable via a persisted cursor in `snapshot_cursors`.
