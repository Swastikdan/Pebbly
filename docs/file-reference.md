...
| `auth.ts`                   | Clerk session handling: `getSessionToken` (Bearer → cookie), `getSessionClaims` (JWT verify, 10 s skew), `requireUser` (resolve-or-create user, race-safe, consolidates duplicates), `getCurrentUser`, `findUserByClaims` (multi-format tokenIdentifier + 15 s / 500-entry LRU cache), admin helpers (`isAdminByClaims` from JWT, `isAdminFromClerkApi` live API 5 s timeout, `getClerkAdminIds`). |
...
| `prompts.ts`                | Pure Gemini prompt builders (`buildWatchlistPrompt`, `buildGenrePrompt`, `buildCustomListPrompt`, `buildHomepageRecommendationsPrompt`) over one shared sectioned builder. Dependency-free; lives server-side because the recommendation fns are its only consumer.                                                                                                                                |
| `ai.ts`                     | `callGeminiAI`, Gemini REST client: 4-model fallback chain, 30 s per-attempt timeout, per-element Valibot validation, high-demand/503 detection, retries.                                                                                                                                                                                                                                          |
...
| `daily-pick-store.ts`     | Offline cache of daily-pick payloads + per-title backdrop/poster paths (~100-entry detail cache).                               |
| `guest-store-kit.ts`      | Shared plumbing: `guestPersistOptions` (LRU storage on client, memory during SSR), `localId`, `nextRank`, `mergeDefinedFields`. |
