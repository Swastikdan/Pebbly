...
| File                     | What it is                                                                                                                                                                                                                                        |
| :----------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `start.ts`               | Client `createStart` instance: custom `serverFns.fetch` attaches a fresh Clerk Bearer token + 30 s abort timeout; TanStack Start's native `createCsrfMiddleware` protects server-fn RPCs from cross-site abuse.                                   |
...
| `fns/rpc.ts`                 | The shared guard pipeline: `authedFn(config, data, handler)` (mode/guest/feature/admin gates, injects typed `{ data, user, claims, db }`). Replaced four hand-written guard idioms across all server fns.                                                                                                                                                                   |
...
| `route-helpers.ts`                        | Loader/head ceremony shared by detail-style routes (`requireRouteId`, `loadMediaRouteData`, `detailHead`) plus typed route destination builders (`mediaDetailRoute`, `tvSeasonRoute`, `tvSeasonsRoute`, `collectionRoute`).                                           |
...
| `repository/types.ts`                     | `WatchlistRepository` + `ListsRepository` interfaces (args objects for `setProgressStatus`/`setReaction`) + `resolveProgressStatusAction`/`resolveStatusPlan`, the shared progress decision pipeline.                                                                 |
| `repository/remote-repository.ts`         | Server-backed repository: optimistic journal, persistent mutation outbox/replay, journaled mutations, and list ops with id swapping; membership writes delegate to `membership-writer.ts`.                                                                            |
| `repository/membership-writer.ts`         | Internal seam owning the watchlist-membership write path: request batching, optimistic-handle resolution, `applyServerState` merge and outbox coalescing. External `Repository` interface unchanged.                                                                  |
...
| `recommendation-options.ts`               | Pure recommendation filtering (`filterRenderedRecommendations`, `isTrackedRecommendation`, `selectUntrackedHistory`) and generation-option builders (`buildGenerateOptions`, `buildGenerateAgainOptions`, `buildGenerateMoreOptions`) shared by the hooks and routes. |
| `recommendation-eras.ts`                  | `ERA_PRESETS` — era labels/year ranges for recommendation filters, kept out of UI components so pure modules can depend on it without a tsx import.                                                                                                                   |
