...
| File                                     | What it is                                                                                                                                                                                                                                                                                                                                |
| :--------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `package.json`                           | App metadata + scripts (`dev`, `build`, `db:*`, `deploy:cf`, `typecheck`, `lint`, `format`), deps (TanStack Start/Router/Query, React 19, Base UI, Drizzle, Valibot, Clerk, h3, Nitro, Tailwind 4, Zustand, better-fetch, lucide) and dev deps (Vite 7, Wrangler, Biome, drizzle-kit, husky, lint-staged). `packageManager: pnpm@10.6.1`. |
...
| `vite.config.ts`                         | Vite build: Nitro plugin, TanStack Start plugin, React plugin with the React Compiler Babel plugin, Tailwind v4, path aliases. Terser minification with `drop_console` in prod, manual vendor chunking (react, tanstack, clerk, icons), `envPrefix: ["VITE_"]` (only `VITE_*` vars reach the client).                                     |
...
| `components.json`                        | Component registry config for the coss ui CLI (shadcn-registry-compatible; the components themselves are Base UI-based).                                                                                                                                                                                                                  |
| `.prettierrc`                            | Prettier fallback config (Biome is the primary formatter).                                                                                                                                                                                                                                                                                |
| `.env.example`                           | Documented env template: Clerk (client + server), TMDB, Gemini, Cloudflare vars. Local dev secrets live in `.dev.vars`; production uses `wrangler secret put`.                                                                                                                                                                            |
...
| `plan/`                                  | Working plans for in-flight refactors (e.g. `PLAN-coss-ui-migration.md`). Historical context, not runtime code.                                                                                                                                                                                                                           |
...
| File                          | What it is                                                                                                              |
| :---------------------------- | :---------------------------------------------------------------------------------------------------------------------- |
| `0000_dear_warbound.sql`      | Initial schema: all tables + indexes + check constraints (`users` still had `is_admin`).                                |
...
| File          | What it is                                                                                                                                                                                                                                                                                                             |
| :------------ | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ci.yml`      | PR checks against the `cloudflare` branch: install → typecheck → lint → build with build-time `VITE_*` vars only (no Cloudflare secrets).                                                                                                                                                                              |
| `deploy.yml`  | Production deploy on push to `cloudflare` (owner-gated): install → typecheck → lint → D1 migrations (prod) → build with `VITE_*` secrets → `wrangler deploy`. Concurrency serializes prod jobs; actions pinned to node24 runtimes (checkout v7, setup-node v7, pnpm/action-setup v6, wrangler-action v4), see ADR-014. |
...
| `helpers/watch-item.ts`     | Shared watch-item logic: `getWatchItem`, `normalizeProgressStatus`/`normalizeReaction`, `buildMetadataPatch` (rating clamped 0–10), `planMembershipRemoval` (detach-vs-delete on watchlist removal), `upsertWatchItem` (race-safe upsert on `(user, tmdb, mediaType)`), `bumpUserRev` + `bumpWatchlistRev`/`bumpListsRev`/`bumpAiRev`/`bumpPermsRev` revision counters. |
| `helpers/episode-sync.ts`   | Episode-progress engine shared by the watchlist fns: `buildEpisodeSyncStatements`, `loadEpisodeRowsByKey` (offset pagination, 500/page), `syncEpisodeProgressRecord` (one read + one batch + rev bump).                                                                                                                                                                 |
...
| `tmdb-schemas.ts`                 | Valibot schemas for every TMDB response shape (~714 lines).                                                                                                                                                      |
...
| `realtime-mutations.ts`           | Per-domain own-mutation counters (`watchlist`/`lists`/`ai`/permissions) recorded on successful server writes, lets `user-sync.tsx` skip redundant refetches for the client's own changes (see ADR-015).          |
| `cross-tab-sync.ts`               | BroadcastChannel fan-out: sibling tabs of the same browser invalidate each other's query groups instantly, no server round trip.                                                                                 |
| `media-types.ts`                  | Canonical `MediaType` module: `"movie" \| "tv"` union, Valibot schema, type guard, route-slug map.                                                                                                               |
| `text.ts`                         | Tiny pure string helpers (`normalizeTitleKey`, `hashString`) shared by prompts and recommendation verification.                                                                                                  |
...
| `repository/status-plan.ts`       | `resolveStatusPlan`, the one decision pipeline both adapters share for progress-status writes (action + optional TMDB season fetch → episode selections).                                                        |
...
| `guest-store-kit.ts`                    | Shared plumbing for guest stores: `guestPersistOptions` (LRU storage on client, memory during SSR), `localId` (collision-free local ids), `nextRank` (append position), `mergeDefinedFields`. |
...
| `use-watchlist.ts`                      | Thin read/mutation hooks delegating to the repository (~174 lines).                                                                                                                           |
| `watchlist/watchlist-optimistic.ts`     | Pure optimistic op builders for watchlist mutations (incl. `buildSeasonEpisodeSelections`).                                                                                                   |
| `custom-lists/list-optimistic.ts`       | Pure optimistic op builders for custom-list CRUD (`beginCreateListOp`, `beginToggleListItemOp`, `beginReorderListItemsOp`, `swapListId`, ...).                                                |
| `use-custom-lists.ts`                   | Custom-list hook (~180 lines; delegates to the repository).                                                                                                                                   |
...
| `watch-progress/use-watch-progress.ts`  | Watch/player progress orchestration (~465 lines): data fetching, progress calculations, mutations.                                                                                            |
...
| `use-daily-pick.ts`                     | Thin hook over `daily-pick-engine.ts` feeding it data and rendering the pick.                                                                                                                 |
| `use-daily-pick-store.ts`               | Zustand daily-pick bookkeeping store.                                                                                                                                                         |
| `use-filtered-watchlist.ts`             | Filter/sort logic for the watchlist page (grid paginates 30/page in the route).                                                                                                               |
| `use-permissions.ts`                    | Client RBAC summary from `getUserFeaturesFn`; refetches on focus and on `permsRev` deltas (no fixed poll), enables automatic sign-out of banned users via `user-sync.tsx`.                    |
| `data-version.ts`                       | `fetchDataVersion`, client fetch for the combined per-user revision counters (watchlist/lists/AI/permissions), polled by `user-sync.tsx` for cross-device realtime.                           |
...
| `use-media-query.ts`                    | SSR-safe media-query hook + `useIsMobile`.                                                                                                                                                    |
| `use-destructive-toast.ts`              | Shared confirmation toast helper for destructive actions.                                                                                                                                     |
...
`spinner.tsx`, `scroll-area.tsx`, `empty.tsx`, `feedback.tsx`, `toast.tsx`
(`ToastProvider` + manager), `pagination.tsx`, plus app-specific primitives:
`image.tsx`, `media-grid.tsx`, `media-skeleton-list.tsx`, `lazy-section.tsx`,
...
`watchlist-grid.tsx` (30-per-page pagination), `watchlist-card.tsx`,
...
(result grid), `recommendation-history.tsx` (history),
